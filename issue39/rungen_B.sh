...
PARMS1='-f 8192 -t 2 -S -d . '
PARMS2='-f 8192 -t 2 -i '1.0'  -d . '
...
function testsession {
    ...
    (set -x; rawspec -g $1 $PARMS1 $STEM 2>&1 | tee -a $LOG)
    if [ $? -ne 0 ]; then
        oops rawspec 1 exit status $?
    fi
    (set -x; rawspec -g $1 $PARMS2 $STEM 2>&1 | tee -a $LOG)
    if [ $? -ne 0 ]; then
        oops rawspec 2 exit status $?
    fi
